...
function getRandomInt(seed, max) {
    let x = Math.abs(Math.sin(seed) * 100); // increase the 100 if there are more than 100 effects
    ...
    format(chat, str, message = null) {
        ...
        if (!this.emotewidths) {
            ...
            for (let i = 0; i < emoteArray.length; i++) {
                if (document.getElementsByClassName("chat-emote-" + emoteArray[i]).length === 0) {
                    break;
                }
                let width = document.getElementsByClassName("chat-emote-" + emoteArray[i])[0].clientWidth;
                let height = document.getElementsByClassName("chat-emote-" + emoteArray[i])[0].clientHeight;
        ...
        return str.replace(this.regex, m => {
            ...
            let goldenModifier = "";
            let goldenModifierInnerEmoteStyle = "";
            ...
            if (!isHalloween() && proc(seed, punish, goldenProcChance)) {
                let goldenEmote = genGoldenEmote(emote, this.emoteheights[emote], this.emotewidths[emote]);
            ...
            const generifyClasses = ["generify-container", "generify-emote-" + emote, GENERIFY_OPTIONS[suffix] || ""];
...
function stringCodeSplitter(str) {
    let indexOne = findNextTick(str);
    if (indexOne !== -1) {
        let beforeFirstTick = str.substring(0, indexOne);
        let afterFirstTick = str.substring(indexOne + 1);
        let indexTwo = findNextTick(afterFirstTick);
        if (indexTwo !== -1) {
            let betweenTicks = afterFirstTick.substring(0, indexTwo).replace(/\r?\n|\r/g, "");
            let afterSecondTick = afterFirstTick.substring(indexTwo + 1);
            let subArray =
...
function stringSpoilerParser(str, isVisible) {
    let classes = isVisible ? "spoiler visible" : "spoiler";
    let indexOne = str.indexOf("||");
    if (indexOne !== -1) {
        let afterTag = str.substring(indexOne + 2);
        let indexTwo = afterTag.indexOf("||");
        if (indexTwo !== -1) {
            let betweenTags = afterTag.substring(0, indexTwo);
            let subString = RegExp("^\\s*$").test(betweenTags)
...
export { EmoteFormatter, GreenTextFormatter, HtmlTextFormatter, MentionedUserFormatter, UrlFormatter, IdentityFormatter, CodeFormatter, SpoilerFormatter };
