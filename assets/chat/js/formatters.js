...
import {GENERIFY_OPTIONS, HALLOWEEN_RANDOM_EFFECTS, HALLOWEEN_RANDOM_DELAYS, HALLOWEEN_BLACKLIST, HAT_BLACKLIST, HAT_SPECIAL_BLACKLIST, GENERIFY_HAT} from './const'
...
function BirthdayHat(width, height, emote) {
    const today = new Date();
...
function GenerifyHat(width, height, emote, suffix) {
    if (!GENERIFY_HAT.includes(suffix)) {
        return "";
    }
    if (HAT_SPECIAL_BLACKLIST[suffix].includes(emote)) {
        return "";
    }
...
class EmoteFormatter {
    ...
    format(chat, str, message=null){
        ...
        return str.replace(this.regex, (m) => {
            ...
            let hat = BirthdayHat(this.emotewidths[emote], this.emoteheights[emote], emote);
            let generifyHat = GenerifyHat(this.emotewidths[emote], this.emoteheights[emote], emote, suffix);
            if (generifyHat != "") {
                hat = generifyHat;
            }
