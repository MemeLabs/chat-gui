    ...
    notificationPermission() {
        return new Promise((resolve, reject) => {
            ...
            if (!isOnWhitelist || Notification.permission === "denied") {
                window.open("notification-request.html", "strimsgg_notification_request", "width=585,height=340,scrollbars=no,toolbar=no");
...
class ChatUserMenu extends ChatMenu {
    ...
    addElement(username, sort = false) {
        ...
        const usr = $(`<a data-username="${user.username}" class="user ${features} ${tag}">${label} <i class="fa fa-share-square whisper-nick"></i></a>`);
...
class ChatWhisperUsers extends ChatMenu {
    ...
    updateNotification() {
        ...
        this.unread = [...this.chat.whispers.entries()].map(e => parseInt(e[1].unread)).reduce((a, b) => a + b, 0);
...
export { ChatMenu, ChatSettingsMenu, ChatUserMenu, ChatEmoteMenu, ChatWhisperUsers };
